    ...
    while (1) {
        ...
        if (sem_trywait(full_prio) == 0) {
            ...
            if (shm->prio_count > 0) {
                ...
                continue;            // Przerywamy petle i wracamy sprawdzic PRIO
            }
...
void manager(SharedData *shm, sem_t *empty_norm, sem_t *full_norm, sem_t *empty_prio, sem_t *full_prio, sem_t *mutex) {
    while (1) {
        sleep(5);
        
        // POCZATEK SEKCJI KRYTYCZNEJ MANAGERA MONITORING
        sem_wait(mutex);
        printf("\n--- [MANAGER] Stan Systemu: NORM: %d/10 | PRIO: %d/10 ---\n", shm->norm_count, shm->prio_count);
        sem_post(mutex);
        // KONIEC SEKCJI KRYTYCZNEJ MANAGERA MONITORING
        
        // zapobieganie starvation - przeniesienie zadania
        // Proba bezpiecznego (nieblokujacego) pobrania elementu z NORMAL
        if (sem_trywait(full_norm) == 0) {
            // Proba bezpiecznego (nieblokujacego) zarezerwowania miejsca w PRIORITY
            if (sem_trywait(empty_prio) == 0) {
                
                // POCZATEK SEKCJI KRYTYCZNEJ MANAGERA przenoszenie
                sem_wait(mutex);
                ...
                sem_post(mutex);
                // KONIEC SEKCJI KRYTYCZNEJ MANAGERA przensozenie
                
                sem_post(empty_norm); // Aktualizacja semaforow po udanym przeniesieniu
                sem_post(full_prio);
            } else {
                sem_post(full_norm); // Brak miejsca w PRIO, zwracamy pobrany zasob z powrotem do NORM
            }
        }
        printf("------------------------------------------------------\n\n");
    }
...
int main() {
    ...
    // Inicjalizacja osobnych zestawow semaforow dla dwoch niezaleznych kolejek oraz muteksu
    ...
    // Tworzenie procesu managera
    if (fork() == 0) { manager(shm, empty_norm, full_norm, empty_prio, full_prio, mutex); exit(0); } // proces menegera
    
    // Proces macierzysty oczekuje na zakonczenie wszystkich dzieci (w tym managera)
